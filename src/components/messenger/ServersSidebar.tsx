...
export default function ServersSidebar({
  servers, dms, selectedServer, selectedDM, viewMode,
  onSelectServer, onDMMode, onAddServer
}: Props) {
    ...
    <div className="w-[72px] flex flex-col items-center py-3 gap-2 flex-shrink-0 relative"
      ...
      <div className="w-12 h-12 flex items-center justify-center mb-1 cursor-pointer animate-float">
        <img
          ...
          alt="GhostNet"
          style={{ width: '40px', height: '40px', objectFit: 'contain', filter: 'drop-shadow(0 0 8px rgba(0,255,231,0.5))' }}
        />
      </div>
      ...
      {/* DMs button */}
      <div className="relative cursor-pointer" onClick={onDMMode}>
        <div
          className={`w-12 h-12 flex items-center justify-center rounded-2xl transition-all duration-200 clip-corner-sm
      ...
      <div className="flex flex-col gap-2 overflow-y-auto flex-1 w-full items-center" style={{ scrollbarWidth: 'none' }}>
        {servers.length === 0 && (
          <div className="flex flex-col items-center gap-1 mt-2 opacity-30 pointer-events-none">
            <Icon name="Server" size={18} className="text-gray-600" />
          </div>
        )}
        {servers.map(server => (
          <div key={server.id} className="relative cursor-pointer" onClick={() => onSelectServer(server)}>
            <div
      ...
      <div className="mt-auto">
        <button
          onClick={onAddServer}
          className="w-12 h-12 flex items-center justify-center rounded-2xl transition-all duration-200 hover:rounded-xl hover:border-cyan-400"
